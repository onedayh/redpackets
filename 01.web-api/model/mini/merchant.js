...
class MerchantModel extends daoBase {
    get getByIdSql() {return "select merchant_name,merchant_logo,merchant_account,merchant_contacter,merchant_mobile,merchant_city,"+
                            "security_key,app_id,security_key,merchant_wxid,merchant_key,merchant_cert,merchant_sms_appid,merchant_sms_appkey  "+
                            "from  mini_relation r inner join mini_merchant m on r.merchant_id=m.merchant_id where r.merchant_id=? and r.mini_id=? ";}
    get updateSql() {return "update mini_merchant set ? where merchant_id=?";}
    get insertSql() {return "insert into mini_merchant set ?";}
    ///更新商户的小程序配置
    updateMiniAppSetting(param){
        let sqlCommand = "call proc_mini_merchantappsetting(?,?,?,?,?,?,?,?);";
        return this.database.executeSql(sqlCommand, [param.id,param.miniid,param.securitykey,param.wxmid,param.wxmkey,param.wxmcert,param.smsappid,param.smsappkey]);
    }
    ///根据登录账号获取商户信息
    getByAccount(account) {
        let sqlCommand="select merchant_name,merchant_account from mini_merchant where merchant_account=?";
        return this.database.executeSql(sqlCommand, account);
    }
    ///根据主键获取一条记录
    getBykey(Sql,id) {
...
exports = module.exports = MerchantModel;